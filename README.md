...
Any Next.js-compatible host works — set the three env vars, run `npm ci && npm run build`, and serve with `npm start`. The `refresh-medium-feed.yml` workflow only runs on GitHub Actions; for other CI, call the script directly with `npm run refresh:medium -- <medium_handle>` (or `MEDIUM_USERNAME=... node scripts/refresh-medium-feed.mjs`).
