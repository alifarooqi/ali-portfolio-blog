...
- No test framework yet; planned in #46 (Vitest + Playwright).

Package manager is **npm** (`package-lock.json`). The README mentions pnpm but the lockfile is npm.

CI (`.github/workflows/ci.yml`) runs `lint` -> `typecheck` -> `build` on every PR and on pushes to `main`. Do not merge with red CI.
...
Despite `next-mdx-remote` and `sugar-high` being listed as dependencies, blog posts are **not** local MDX. `lib/medium.ts` fetches the Medium RSS feed for `@ali_farooqi`, caches it in-memory for 12h (matching `revalidate = 12 * 3600` on the blog routes), and falls back to the committed snapshot at `lib/medium-feed.json` if the fetch fails. `app/blog/[slug]/page.tsx` renders the post's raw HTML via `dangerouslySetInnerHTML`. To refresh the fallback snapshot, regenerate it from https://rss2json.com (see the comment at the top of `medium.ts`).
...
- `next` is on the `canary` tag; React is pinned at 18.2.0.
